...
#     print(f"welcome {name}, class: {class_name}")
#     return 
# name_students = input("enter your name: ")
# Class_students = input("please enter your class: ")
# print_name_tag(name_students, Class_students)
...
#     moenys = 120 * nights
#     print(f"booking vonfirmed for {guest_name}, number of nights {nights}, total cost {moenys}")
# nightz = int(input("how many nights are u gonna stay :"))
# guestz_name = input("please eneter your name :")

# booking_confirmation(guestz_name, nightz)
...
#     if temperature >= 32:
#         advise = "Hot"
#     elif temperature <= 31 and temperature >= 25:
#         advise = "Warm"
#     else:
#         advise = "cool"
#     print(f"city : {city}, temperature : {temperature}°C, advisory : {advise}")
# temperaturez = int(input("please enter the current temperature :"))
# cityz = input("please enter the city: ")
# weather_report(cityz, temperaturez)
...
def validate_date(date_str):
    if not date_str[0:2].isdigit():
        print('Day must be a digit')
        return False
    if not date_str[3:].isdigit():
        print("month must be a digit ")
        return False
    if len(date_str) != 5:
        print("Invalid format")
        return False
    if  not int(date_str[0:2]) <=30 or not int(date_str[0:2])>=1:
        print("DD must be within 1 to 30")
        return False
    if date_str[2] != "-":
        print("There must be a - separating the DD and MM")
        return False
    if not int(date_str[3:]) <= 12 or not int(date_str[3:]) >=1:
