...
#     file.write("Today I learnt how to write to a filez")
...
#     file.write("\nI also revised python at night")
# with open("q3_diary.txt","r") as filez:
#     content = filez.read()
#     print(content)
...
#     content = file.readlines()
#     marks =[]
#     for markz in content:
#         marks.append(int(markz))
#     min = marks[0]
#     max = marks[0]
#     for num in range(len(marks)): # 75, 62 marks[75]
#         sum += marks[num]
#         if marks[num] < min:
#             min= marks[num]
#         else:
...
with open("proper_names.txt", "a") as fobj:
    for name in content:
        fobj.write(name.title()+"\n")
...
with open("q7_temperatures.txt","r") as file:
    content = file.readlines()
    for num in content:
        if int(num) > 30: # num >> string
            count += 1
            temp.append(num)
with open("hot_days.txt","w") as filezs:
    filezs.writelines(temp)
